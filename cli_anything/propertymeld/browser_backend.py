...
def _browser_session(creds: dict):
    ...
    context = browser.new_context(viewport={"width": (128 * 10), "height": 900})
    ...
    try:
        ...
        for card in page.locator("button, a").all():
            try:
                ...
                if "Nexus Partner" in txt and (account_token := os.environ.get("PM_ACCOUNT_MATCH_TOKEN", "")) and account_token in txt and len(txt) < 60:
                    card.click()
                    time.sleep(5)
                    nexus_clicked = True
                    break
