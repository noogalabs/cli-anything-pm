...
def assign_tech(meld_id: str, tech_name: str) -> dict:
    ...
    Args:
        ...
        tech_name: Partial name match (case-insensitive). e.g. "Jordan" or "Jordan Lee".
    """
    meld_id = _validate_meld_id(meld_id)
    creds = _load_creds()
    cookie_hdr = _cookie_header(creds)
...
def complete_meld(
    ...
    *,
    ...
    meld_id = _validate_meld_id(meld_id)
    if side != "vendor":
        # Alex's invariant is absolute: our tooling must never write a
...
def create_project(
    ...
    Required (per capture):
        ...
        coordinators (list of management-agent int ids, e.g. [51001]),
...
def list_agents() -> list:
    ...
    silent misread class where in-house techs (Jordan / Casey / Silvano / etc)
